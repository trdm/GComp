...
CodeWorks.pm - модуль (package) с базовыми функциями для обработки кода.
Методы: 
	...
	RemoveVarDecl($ObjName, $ProcName, $VarName) - удаляет обявление переменной. Параметры аналогичны InsertVarDecl()
	...
	ReplaceCode($ObjName, $ProcName, $OldCode, $NewCode) - замена одной строки кода на другую.
		...
		- $OldCode  - Старый код. RegExp шаблон, который должен матчить всю строку, исключая
		              лидирующие и конечные пробелы. Шаблон не должен содержать группировок - если группировки
					  всё-же нужны, то используйте non capturing groupings, т.е. что-то вроде (?:Сам|Этот|This|Self).
...
Code_Ins.pl  - скрипт для массовой правки кода во всех модулях.
Работает так: перебирает все файлы *.1s, для каждого определяет шаблон, и применяет этот шаблон.
Шаблоны - это просто скрипты, использующие методы CodeWorks.pm. Каталог с шаблонами 
указывается в командной строке. Имена шаблонов могут быть такими:
	МодульДокумента.pl
	ФормаДокумента.pl
	ФормаГруппы.pl
	ФормаСписка.pl
